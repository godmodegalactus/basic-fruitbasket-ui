...
const getTokenAmountInPool = async(ctx : UIContext, index : number) => {
    ...
    return (await ctx.tokens[index].getAccountInfo(new web3.PublicKey(token_data.pools))).amount;
}

export const getBasketDataForUser = async(ctx: UIContext, index: number, _userKey: string) => {
  let account = data.basket_accounts[index];
  return {
    key : account,
    balance : await getTokenAmountFromMint(ctx.basket_mints[index], new web3.PublicKey(account)),
  };
}

export const getUserUsdcAccountAndAmount = async(ctx: UIContext, userKey : string) => {
  let account = data.usdc_account;
  return {
    key : account,
    balance : await getTokenAmountFromMint(ctx.usdc, new web3.PublicKey(account)),
  };
}
  ...
  let tx1 = await program.transaction.initTradeContext(
    ...
    new anchor.BN(price_min_or_max),
    {
      accounts: {
        ...
        quoteTokenTransactionPool: new PublicKey(data.quote_token_transaction_pool),
      ...
      const ttx = program.transaction.processTokenForContext(
        {
          accounts : {
            ...
            quoteTokenMint : new PublicKey(data.quote_token_mint),
            basketTokenMint : new PublicKey(data.baskets[basketIndex].basket_mint_address),
